...
\tag{HJ}
\left\{ \begin{array}{ll}
\partial_t u + H(D_x u) = 0, & \text{in} \ [0,T]\times \R^n, \\
u(0,x) = u_0(x), & \text{in} \ \R^n,
\end{array}\right.
$$
where $u_0\in C^{0,1}(\mathbb{R}^n)$ and the Hamiltonian $H: \R^n\rightarrow\R$ is assumed to satisfy the following hypotheses:
$$
\tag{2}
H\in C^2(\R^n), \quad H_{pp}(p) >0, \ \forall p\in\R^n, \quad \text{and} \quad 
\lim_{|p|\to\infty} \dfrac{H(p)}{|p|} = +\infty.
$$
Here, the unknown $u$ is a function $[0,T]\times\R^n\longrightarrow\R$,
...
$$
\begin{array}{cccl}
S^+_T : & C^{0,1}(\R^n) & \longrightarrow & C^{0,1}(\R^n) \\
...
<center>
<figure>
 <img src="01_Fig11.png" alt="1D unreachable targets" height="250" width="650"> 
</figure>
</center>
...
<center>
<figure>
 <img src="07_Fig21.png" alt="2D unreachable targets" height="200" width="600"> 
</figure>
</center>
...
$$
I_T (u_T) := \left\{ u_0\in C^{0,1}(\R^n) \, ; \ S_T^+ u_0 = u_T\right\}
$$
...
**Theorem 1:** Let $H$ satisfy (2), $u_T\in C^{0,1}(\R^n)$ and $T>0$. Then, the set $I_T(u_T)$ is nonempty 
...
In the case the observation $u_T$ at time $T$ is **not admissible**, maybe due to noise effects or to errors in the measurements, we can actually project it on the set of admissible observations by applying the operator $S_T^+\circ S_T^-$. For a given $u_T\in C^{0,1}(\R^n)$, we define this projection as follows:
$$
u_T^\ast := S_T^+(S_T^- u_T).
$$
Observe that $I_T(u_T^\ast)\neq \emptyset$ for any $u_T\in C^{0,1}(\R^n)$. 
Indeed, $\tilde{u}_0:=S_T^- u_T\in I_T (u_T^\ast)$ by definition.

<center>
<figure>
 <img src="02_Vid11.gif" alt="1D backward-forward" height="250" width="350"> 

 <img src="02_Vid12.gif" alt="1D backward-forward" height="250" width="350"> 
</figure>
</center>
...
<center>
<figure>
 <img src="08_Vid41.gif" alt="2D backward-forward" height="300" width="400"> 

 <img src="08_Vid42.gif" alt="2D backward-forward" height="300" width="400"> 
</figure>
</center>
...
Let $u_T\in C^{0,1}(\R^n)$ and define the functions
$$
u_T^\ast := S_T^+(S_T^- u_T) \qquad \text{and} \qquad \tilde{u}_0 := S_T^- u^\ast_T.
$$
Then, for any $u_0\in C^{0,1} (\R^n)$, the two following statements are equivalent:
1. $u_0\in I_T(u_T^\ast)$;
2. $u_0(x)\geq \tilde{u}_0 (x), \ \forall x\in \R^n \quad \text{and} \quad u_0(x) = \tilde{u}_0(x), \ \forall x\in X_T(u_T^\ast),$

where $X_T(u_T^\ast)$ is the subset of $\R^n$ given by 
$$
X_T(u_T^\ast) : = \left\{ z- T\, H_p (\nabla u_T^\ast(z)); \ \forall z\in\R^n \ \text{such that} \ 
...
- and the set $X_T(u_T^\ast)\subset \R^n$, that can be deduced from the differentiability points of $u_T^\ast$.
...
<center>
<figure>
 <img src="03_Fig12.png" alt="1D reconstruction 1" height="250" width="550"> 
</figure>
</center>
...
<center>
<figure>
 <img src="04_Fig13.png" alt="1D reconstruction 2" height="220" width="550"> 
</figure>
</center>
...
$$
I_T(u_T) = \left\{ \tilde{u}_0 + \varphi \, ; \, \varphi\in C^{0,1}(\R^n)\ 
\text{such that} \ \varphi\geq 0\ \text{and}\ \text{supp} (\varphi) \subset \R^n\setminus X_T(u_T)\right\}.
$$
...
<center>
<figure>
 <img src="05_Vid2.gif" alt="1D random examples" height="300" width="400"> 
 <img src="06_Vid3.gif" alt="1D random examples 2" height="300" width="400"> 
</figure>
</center>
...
while in the region $\R^n\setminus X_T(u_T^\ast)$, only a lower bound can be deduced.
...
In this case it holds that $X_T(u_T)=\R^n$.
...
Observe the different structure of the set $\R^n\setminus X_T(u_T^\ast)$ (white region in the plot at the right)
in the surrounding area of bumps and wells of $u_T^\ast$.
- Near a bump, the set where $u_T^\ast$ is not differentiable is an isolated point, and then, it produces a ball for the set $\R^n\setminus X_T(u_T^\ast)$.
- Near a well, $u_T^\ast$ is non-differentiable on a circumference. This produces an annulus for the set $\R^n\setminus X_T(u_T^\ast)$.

<center>
<figure>
 <img src="09_Fig22.png" alt="2D reconstruction 1" height="200" width="700"> 
</figure>
</center>
...
<center>
<figure>
 <img src="10_Fig23.png" alt="2D reconstruction 2" height="200" width="700"> 
</figure>
</center>
...
<center>
<figure>
 <img src="11_Fig31.png" alt="1D initial data" height="200" width="700"> 
</figure>
</center>
...
<center>
<figure>
 <img src="12_Vid5Gif.gif" alt="1D reconstruction evolution" height="200" width="700"> 
</figure>
</center>
...
<center>
<figure>
 <img src="13_Vid6Gif.gif" alt="1D reconstruction evolution2" height="200" width="700"> 
</figure>
</center>
...
<center>
<figure>
 <img src="14_Fig41.png" alt="2D initial data" height="200" width="700"> 
</figure>
</center>

<center>
<strong> Figure 8: </strong> Two initial data in dimension 2.
</center> 
...
the white region, which corresponds to $\R^n\setminus X_T(u_T)$ becomes bigger and bigger.
...
<center>
<figure>
 <img src="15_Vid7Gif.gif" alt="2D reconstruction evolution 1" height="250" width="750"> 
</figure>
</center>
...
<center>
<figure>
 <img src="16_Vid8Gif.gif" alt="2D reconstruction evolution 2" height="250" width="750"> 
</figure>
</center>
...
<center>
<figure>
 <img src="17_Vid9Gif.gif" alt="2D reconstruction evolution 3" height="250" width="750"> 
</figure>
</center>
