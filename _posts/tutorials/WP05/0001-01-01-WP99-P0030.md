...
<p> We now present some numerical siimulation of the controlled trajectory. We shall consider the case  $A = 0$
and $B = 1,$ i.e the classical Lotka-McKendrick system. We also consider the case $a_1 = 0$ and $a_2 = a_{\dagger} =4,$ i.e., the control
acts everywhere with respect to age variable. 
Then by Theorem 1 we know that system (1) is null controllable in any time. In the following
